...
export class AddonNotesListPage implements OnDestroy {
    ...
    constructor(private translate: TranslateService, private calendarProvider: AddonNotesProvider, private navParams: NavParams,
            private domUtils: CoreDomUtilsProvider, private coursesProvider: CoreCoursesProvider, private utils: CoreUtilsProvider,
            private calendarHelper: AddonNotesHelperProvider, private sitesProvider: CoreSitesProvider,
        ...
        if (this.notificationsEnabled) {
            ...
            this.obsDefaultTimeChange = eventsProvider.on(AddonNotesProvider.DEFAULT_NOTIFICATION_TIME_CHANGED, () => {
    ...
    fetchEvents(refresh = false) {
        return this.calendarProvider.getEventsList(this.daysLoaded, AddonNotesProvider.DAYS_INTERVAL).then((events) => {
            this.daysLoaded += AddonNotesProvider.DAYS_INTERVAL;
            if (events.length === 0) {
     ...
     */
    openSettings() {
        this.navCtrl.push('AddonNotesSettingsPage');
    }
    ...
    gotoEvent(eventId) {
        ...
        this.splitviewCtrl.push('AddonNotesEventPage', {id: eventId});
    }
